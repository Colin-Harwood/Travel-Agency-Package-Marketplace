...
switch ($action){
    ...
    case 'get_restaurants':
        getRestaurants($conn);
        break;
    case 'get_complete_packages':
        getCompletePackages($data, $conn);
        break;
    default:
...
function getCompletePackages($data, $conn){
    $agencyID = $data['agencyID'] ?? null;
    ...
    $stmt = $conn->prepare("SELECT * FROM package WHERE agencyID = ?");
    ...
    while($package = $result->fetch_assoc()){
        ...
        $stmtFlights = $conn->prepare(
            "SELECT f.*
            FROM PACKAGE_FLIGHT pf
            INNER JOIN FLIGHT f
            ON pf.flightID = f.flightID
            WHERE pf.packageID = ?"
        );

        $stmtFlights->bind_param("i", $packageID);
        $stmtFlights->execute();

        $flightResult = $stmtFlights->get_result();
        $flights = [];
        ...
        $stmtAccommodation = $conn->prepare(
            "SELECT a.*
            FROM PACKAGE_ACCOMMODATION pa
            INNER JOIN ACCOMMODATION a
            ON pa.accommodationID = a.accommodationID
            WHERE pa.packageID = ?"
        );

        $stmtAccommodation->bind_param("i", $packageID);
        $stmtAccommodation->execute();

        $accommodationResult = $stmtAccommodation->get_result();
        $accommodations = [];
        while($row = $accommodationResult->fetch_assoc()){
            $accommodations[] = $row;
        }

        $stmtRestaurant = $conn->prepare(
            "SELECT r.*
            FROM PACKAGE_RESTAURANT pr
            INNER JOIN RESTAURANT r
            ON pr.restaurantID = r.restaurantID
            WHERE pr.packageID = ?"
        );

        $stmtRestaurant->bind_param("i", $packageID);
        $stmtRestaurant->execute();

        $restaurantResult = $stmtRestaurant->get_result();
        $restaurants = [];
        while($row = $restaurantResult->fetch_assoc()){
            $restaurants[] = $row;
        }

        $stmtAttraction = $conn->prepare(
            "SELECT a.*
            FROM PACKAGE_ATTRACTION pa
            INNER JOIN ATTRACTION a
            ON pa.destinationID = a.destinationID
            WHERE pa.packageID = ?"
        );

        $stmtAttraction->bind_param("i", $packageID);
        $stmtAttraction->execute();

        $attractionResult = $stmtAttraction->get_result();
        $attractions = [];
        ...
        $package['accommodations'] = $accommodations;
        $package['restaurants'] = $restaurants;
        $package['attractions'] = $attractions;
